...
const DashboardRenderer = {
            ...
            <div class="dashboard-container">
                ...
                <div class="tracking-config-panel">
                    ...
                    <div class="tracking-config-grid">
                        ${this.renderTrackingConfig('DRP', config.trackingMode.DRP)}
                        ${this.renderTrackingConfig('RRC', config.trackingMode.RRC)}
                        ${this.renderTrackingConfig('FAT', config.trackingMode.FAT)}
                        ${this.renderTrackingConfig('PMB', config.trackingMode.PMB)}
                        ${this.renderTrackingConfig('PUB', config.trackingMode.PUB)}
                        ${this.renderTrackingConfig('DJK', config.trackingMode.DJK)}
                    </div>
                </div>
    ...
    renderWeeksList() {
        ...
        return weeks.map(week => {
            ...
            return `
                ...
                `dod-${week.week_id}-${i}`,
                'dod',
                { weekId: week.week_id, dodId: `dod${i}`, module: 'DRP' },
                dod
            )).join('')}
                        </div>
                    ` : ''}
    ...
    resetAllData() {
        if (confirm('⚠️ ¿ESTÁS SEGURO? Esta acción borrará TODO el progreso registrado (Tareas, DoDs, Evidencias RA). No se puede deshacer.')) {
            // Limpiar claves de almacenamiento
    ...
    renderCompetenciesChart(stats) {
        ...
        this.charts.competencies = new Chart(ctx, {
            type: 'radar',
            data: {
                labels: comps,
                datasets: [{
    ...
    renderEvaluationsChart(stats) {
        ...
        this.charts.evaluations = new Chart(ctx, {
            type: 'bar',
            data: {
                labels: ['E1: Estantería', 'E2: Taburete', 'E3: Mobiliario'],
                datasets: [{
                    label: 'Completado %',
                    data: data,
